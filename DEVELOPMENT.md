...
| Command | Purpose |
| --- | --- |
| `npm run build` | Build renderer, Clipdip, launcher, vendored binaries, ingest-key file, fetch shared FFmpeg, and build the Windows installer. |
...
without importing Electron; `verify()` returns the ffmpeg version line or names
the missing file. Clipdip's `fetch-ffmpeg` script delegates to the root one and
its standalone zip takes the same binaries. On start ClipLib writes the shared
path into clipdip's `output.ffmpeg_path`, migrating paths it wrote in earlier
versions (ffmpeg-static, resources) and leaving user overrides alone.

`npm run test:ffmpeg` exercises the real export, probe, screenshot, audio analysis
and recorder-path code against the bundled binary and prints a timing table.
`cargo test -p clipdip-muxer -- --nocapture` does the same for the clip mux.

The ffmpeg tests need the existing `vendor/ffmpeg/` cache and installed dependencies;
they do not download anything. `npm test` also runs this suite. The Windows directory
entry point is `test/ffmpeg/index.js`. Electron is stubbed with a temporary profile
...
cleared between samples. Export timings include production probing and bookkeeping.
Audio analysis returns per-track RMS/peak envelopes and integrated loudness for
the mix, not separate integrated LUFS for every track.

Set `CLIPLIB_FFMPEG_BASELINE` to another standalone `ffmpeg.exe` to compare it with
the bundled build. Its sibling `ffprobe.exe` is used unless
`CLIPLIB_FFPROBE_BASELINE` names another file. The harness copies that pair to a
temporary directory and runs the same operations in a child process. Both builds
use fixtures generated by the bundled binary. The ratio is bundled time divided
by baseline time; values below one mean the bundled build finished sooner.
`CLIPLIB_FFMPEG_DIR` overrides the resolver directory for these child processes;
leave it unset when validating the bundled build. `isBundled` still describes
whether a packaged binary was found, independently of that directory override.
