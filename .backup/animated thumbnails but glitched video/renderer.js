  ...
  return new Promise(async (resolve) => {
    ...
    clipElement.addEventListener('mouseenter', () => {
      hoverTimeout = setTimeout(() => {
        ...
        clipElement.insertBefore(videoElement, clipElement.firstChild);

        videoElement.currentTime = clip.isTrimmed ? trimStartTime : 0;
...
async function openClip(originalName, customName) {
  ...
  if (trimData) {
    window.trimStartTime = trimData.start;
    window.trimEndTime = trimData.end;
  } else {
    window.trimStartTime = 0;
    window.trimEndTime = clipInfo.format.duration;
  }
...
function setupClipTitleEditing() {
  ...
  if (clipTitleElement) {
    clipTitleElement.addEventListener("focus", () => {
      isRenamingActive = true;
    });

    clipTitleElement.addEventListener("blur", () => {
      isRenamingActive = false;
      saveTitleChange();
    });

    clipTitleElement.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        clipTitleElement.blur();
      }
    });
  }
}
...
function closePlayer() {
  ...
  // Update the clip's display in the grid if currentClip exists
  if (currentClip) {
    updateClipDisplay(currentClip.originalName);
  }
...
function handleKeyPress(e) {
  ...
  if (e.key === ' ') {
    // if videoplayer has src, then toggle play/pause
    if (videoPlayer.src) {
      e.preventDefault();
      togglePlayPause();
    }
...
async function saveTitleChange() {
  ...
  try {
    ...
    await showCustomAlert(`Failed to save custom name. The file might be in use or you may not have the necessary permissions. Please try again later. Error: ${error.message}`);
